...
class ProductDetailScreen(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.product_image_label.pack(pady=5) # Reduced padding

        # --- Price and Sold Count ---
        price_sold_frame = tk.Frame(self, bg=LIGHT_BG)
        price_sold_frame.pack(fill="x", padx=15, pady=3) # Reduced padding
        ...
        # --- Product Name ---
        self.name_label = tk.Label(self, text="", font=TITLE_FONT, fg=GRAY_TEXT, bg=LIGHT_BG, wraplength=280, justify="left") # Reduced wraplength
        self.name_label.pack(fill="x", padx=15, pady=(0, 5)) # Reduced padding

        # --- Separator Line ---
        tk.Frame(self, bg=BORDER_COLOR, height=1).pack(fill="x", padx=10, pady=5) # Reduced padding

        # --- Details Section (Guaranteed, Free Return, etc.) ---
        details_frame = tk.Frame(self, bg=LIGHT_BG)
        details_frame.pack(fill="x", padx=15, pady=3) # Reduced padding

        tk.Label(details_frame, text="Guaranteed to get by : 17-19 May", font=GLOBAL_FONT, fg=GRAY_TEXT, bg=LIGHT_BG).pack(anchor="w", pady=1) # Reduced padding
        tk.Label(details_frame, text="Free & Easy Return", font=GLOBAL_FONT, fg=GRAY_TEXT, bg=LIGHT_BG).pack(anchor="w", pady=1)
        tk.Label(details_frame, text="Merchandise Protection", font=GLOBAL_FONT, fg=GRAY_TEXT, bg=LIGHT_BG).pack(anchor="w", pady=1)

        # --- Separator Line ---
        tk.Frame(self, bg=BORDER_COLOR, height=1).pack(fill="x", padx=10, pady=5) # Reduced padding

        # --- Select Variation (Placeholder) ---
        tk.Label(self, text="Select variation", font=GLOBAL_FONT_BOLD, fg=PUP_RED, bg=LIGHT_BG).pack(anchor="w", padx=15, pady=(0, 3)) # Reduced padding
        
        variation_frame = tk.Frame(self, bg=LIGHT_BG)
        variation_frame.pack(fill="x", padx=15, pady=(0, 5)) # Reduced padding

        tk.Label(variation_frame, text="36", font=GLOBAL_FONT, fg=GRAY_TEXT, bg=WHITE_BG, width=4, relief="solid", bd=1).pack(side="left", padx=3) # Reduced width/padding
        ...
        rating_frame.pack(fill="x", padx=15, pady=(0, 10)) # Reduced padding

        tk.Label(rating_frame, text="4.9", font=GLOBAL_FONT_BOLD, fg=PUP_RED, bg=LIGHT_BG).pack(side="left")
        tk.Label(rating_frame, text="★", font=(GLOBAL_FONT[0], 12), fg=PUP_GOLD, bg=LIGHT_BG).pack(side="left", padx=(0, 3)) # Reduced font size/padding
        ...
        button_actions_frame.pack(fill="x", pady=5) # Reduced padding

        # Load button images (ADJUSTED SIZE)
        ...
        self.add_to_cart_button.pack(side="left", padx=(10, 5)) # Reduced padding
        ...
        self.buy_now_button.pack(side="left", padx=(5, 0)) # Reduced padding
    ...
    def load_product(self, product_id):
        ...
        if product_data:
            ...
            self.product_display_image = load_image(product_image_full_path, (180, 180)) # Reduced image size
